## Version Information
This manual applies to **MOHID SOIL TOOL [version 4.1.0]**.  <br><br>
...
14. [Using Other Raster Sources](#using-other-raster-sources)  
...
1. Access the [download link](https://github.com/dhiegosales/MOHID-SOIL-TOOL/releases/download/v4.1.1/MOHID_SOIL_TOOL_version4_1_1.zip).
...
- **Importing Files:** Users import watershed shapefiles (*.shp) and soil texture rasters (sand, silt, clay, bulk density *.tif).
   ...
   - Soil map:
      ...
      - USER: The user can import their own shapefile containing soil map polygons. In this case, the user must specify the column in the 'Attribute Table' that contains the soil type data.
   ...
   - Layers raster: the user can load up to 6 layer files, including:
      - and raster 
      - silt raster 
      - clay raster 
      - Density raster 
...
![image](https://github.com/dhiegosales/MOHID-SOIL-TOOL/blob/main/v4.1.1_Algorithm.png)
...
![MST GUI](https://github.com/dhiegosales/MOHID-SOIL-TOOL/blob/main/v4.1.1_and_above_GUI.png)
...
## Example Data
To facilitate the correct use of the **MOHID Soil Tool (MST)**, we provide example datasets and processed outputs that can be used as reference material.  
These files allow the user to test the workflow and compare their own results with validated outputs generated by MST.

The input data and the results obtained by the MOHID Soil Tool (MST) can be accessed in the dataset created by:  

**da Silva Sales, D. (2025): Supplementary Soil Dataset for Enhancing River Flow Predictions in MOHID-Land Through Integration of Gridded Soil Data and Hydraulic Parameters Using the MOHID Soil Tool (1.1.0) [Data set]. Zenodo. https://doi.org/10.5281/zenodo.14914611**  

This dataset incorporates soil data from EMBRAPA and was used in the paper *"Enhancing River Flow Predictions in MOHID-Land Through Integration of Gridded Soil Data and Hydraulic Parameters Using the MOHID Soil Tool."*  

➡️ The example files available include:  
- Soil type shapefiles.  
- Raster layers (sand, silt, clay, and bulk density).  
- Processed shapefiles with soil texture classification.  
- ASCII input files for MOHID-Land.  
- Excel reports with detailed parameter tables.  

These resources can be consulted to verify that the MST is running correctly on your system and to compare your outputs with standardized reference results.

[Back to Top](#table-of-contents)

### Using Other Raster Sources
- Any raster dataset in **GeoTIFF (.tif)** format can be used, provided it includes the necessary soil properties: **sand, silt, clay, and bulk density**.  
- This flexibility allows MST to be applied in **any region of the world** where soil property rasters are available, making it suitable for diverse hydrological and land modeling projects.  
- While the tool is optimized for EMBRAPA and Brazilian soil maps, it has been successfully adapted for use with international datasets.  
